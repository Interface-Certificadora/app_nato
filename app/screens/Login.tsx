...
import termos from "@/api/termos/get";
  ...
  async function handleRequest() {
    try {
      ...
      await Api.patchemail(emailjson, clienteData.id);
          const aceitouTermos = await termos(clienteData.id);
          console.log("registro de termos",aceitouTermos);
          if (aceitouTermos) {
            router.push("./Logs");
          } else {
            router.push("./Termos");
...
const styles = StyleSheet.create({
  ...
  btn: { backgroundColor: "#23CF5C", borderRadius: 50, padding: 7, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 3.84, elevation: 5 },
